...
/**
 * Add column sort to database table
 */
if( isset( $_POST[$cm->get('request_key')]['db']['submit'] ) )
{
    if( $cm->checkCategoriesDbTable() === false
        && $cm->addColumnToCategoryDbTable() === true )
    {
        $cm->addMessage( $cm->txt('The column "rank" was successfully set.'), 'global' );
        $cm->addMessage( $cm->txt('Please <a href="index.php?a=108&amp;id=' . $_GET['id'] . '">go to Module configuration</a> and set <span class="highlight">"check_db_table"</span> to <span class="highlight">false</span>'), 'global' );
    }
}
